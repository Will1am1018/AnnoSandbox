...
class Process(object):
    ...
    def is32bit(self, pid=None, process_name=None, path=None):
        ...
        if count != 2:
            raise anno sandboxError("Invalid usage of is32bit, only one identifier "
                              "should be specified")
        ...
        # make the System32 binary 64-bit).
            ...
            raise anno sandboxError("File not found: %s" % path)
        else:
        ...
        try:
            ...
            raise anno sandboxError("Error returned by is32bit: %s" % e.output)
    ...
    def inject(self, dll=None, apc=False, track=True, mode=None):
        ...
        @param dll: anno sandbox DLL path.
