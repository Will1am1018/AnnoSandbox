...
def update_conf(machinery, args, action=None):
    ...
    path = os.path.join(anno sandbox_ROOT, "conf", "%s.conf" % machinery)
...
def main():
    ...
    if args.add:
        ...
        update_conf(conf.anno sandbox.machinery, args, action="add")

    if args.delete:
        # TODO Add a db.del_machine() function for runtime modification.
        update_conf(conf.anno sandbox.machinery, args, action="delete")
