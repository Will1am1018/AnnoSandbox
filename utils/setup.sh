...
usage() {
    ...
    echo "-C --clean:        Clean the anno sandbox setup."
...
if [ "$CLEAN" -ne 0 ]; then
    yes|sudo -u anno sandbox -i vmcloak-removevms
    umount /home/anno sandbox/vmmount
    rm -rf /home/anno sandbox/{.config,.vmcloak,vmmount}
    rm -rf /home/anno sandbox/{anno sandbox,anno sandbox.git}
    rm -rf "$BASEDIR/vms" "$BASEDIR/vmbackup"
    rm -rf /opt/anno sandbox
    exit 0
fi
...
_clone_anno sandbox() {
    ...
    # Fetch anno sandbox or in the case of a longterm setup, longanno sandbox.
    if [ "$LONGTERM" -eq 0 ]; then
        sudo -u anno sandbox -i \
            git clone --bare https://github.com/anno sandboxsandbox/anno sandbox.git

        gitrepo="anno sandbox.git"
    else
        sudo -u anno sandbox -i \
            git clone --bare https://github.com/jbremer/longanno sandbox.git

        gitrepo="longanno sandbox.git"
    fi

    sudo -u anno sandbox -i tee "$gitrepo/hooks/post-receive" << EOF
#!/bin/bash
...
GIT_WORK_TREE=/opt/anno sandbox git checkout -f production
EOF

    mkdir -p "/opt/anno sandbox"
    chown anno sandbox:anno sandbox "/opt/anno sandbox"
    chmod 755 "/home/anno sandbox/" "/opt/anno sandbox"
    sudo -u anno sandbox -i chmod +x "$gitrepo/hooks/post-receive"

    # Checkout master branch of the repository (if this was not done already).
    if [ -z "$(ls -A /opt/anno sandbox)" ]; then
        sudo -u anno sandbox -i \
            git --work-tree /opt/anno sandbox --git-dir "$gitrepo" checkout -f master
    fi

    # Add the Suricata reboot crontab entry.
    if ! grep suricata.sh <(crontab -l); then
        (crontab -l ; echo @reboot /opt/anno sandbox/utils/suricata.sh)|crontab -
    fi

    # Delete the anno sandbox1 machine that is included in the VirtualBox
    # configuration by default.
    sudo -u anno sandbox -i "/opt/anno sandbox/utils/machine.py" --delete anno sandbox1
}
...
_setup() {
    ...
    wget "http://anno sandbox.sh/vmcloak-files/${PRETTYNAME}.vbox-extpack"
    ...
    # Setup a anno sandbox user.
    useradd anno sandbox -d /home/anno sandbox -s /bin/bash
    mkdir -p /home/anno sandbox
    chown anno sandbox:anno sandbox /home/anno sandbox

    # Copy any authorized keys from the current user to the anno sandbox user.
    mkdir -p /home/anno sandbox/.ssh
    cp ~/.ssh/authorized_keys /home/anno sandbox/.ssh/authorized_keys
    chown anno sandbox:anno sandbox /home/anno sandbox/.ssh/authorized_keys

    # Add the www-data user to the anno sandbox group.
    adduser www-data anno sandbox
    ...
    VMTEMP="$(mktemp -d "/home/anno sandbox/tempXXXXXX")"

    chown anno sandbox:anno sandbox "/home/anno sandbox/"
    chown -R anno sandbox:anno sandbox "$VMTEMP"
    chmod 755 "/home/anno sandbox/" "$VMTEMP"

    # Install required packages part two.
    pip install --upgrade mitmproxy psycopg2 vmcloak

    # Clone the anno sandbox repository and initialize it.
    _clone_anno sandbox

    # Install required packages part three.
    pip install -r "/opt/anno sandbox/requirements.txt"

    # Create a random password.
    # PASSWORD="$(pwgen -1 16)"
    PASSWORD="anno sandbox"

    sql_query() {
        echo "$1"|sudo -u postgres psql
    }

    sql_query "DROP DATABASE anno sandbox"
    sql_query "CREATE DATABASE anno sandbox"

    sql_query "DROP USER anno sandbox"
    sql_query "CREATE USER anno sandbox WITH PASSWORD '$PASSWORD'"

    # Install the Upstart/SystemV scripts.
    /opt/anno sandbox/utils/service.sh -c /opt/anno sandbox install

    # Fetch the community signatures and monitoring binaries.
    sudo -u anno sandbox -i /opt/anno sandbox/utils/community.py -wafb master
...
EOF
    ...
    # Increase the file descriptor limits. TODO How to set it for just the anno sandbox
    # user? Doesn't seem to work when using 'anno sandbox' instead of '*'.
...
EOF
    ...
    if [ "$LONGTERM" -ne 0 ]; then
        CRONJOB="/home/anno sandbox/vmprovision.sh"

        # Install the machine cronjob.
        "/opt/anno sandbox/utils/experiment.py" machine-cronjob install \
            "cpucount=$CPUCOUNT" "path=$CRONJOB" "basedir=$BASEDIR"
        chown anno sandbox:anno sandbox "$CRONJOB"
        chmod +x "$CRONJOB"

        # We want to run the vm provisioning cronjob every five minutes.
        # Ensure that we only install the cronjob entry once.
        CRONTAB="$(crontab -u anno sandbox -l)"
        if [[ ! "$CRONTAB" =~ "vmprovision.sh" ]]; then
            (echo "$CRONTAB" ; echo "*/5 * * * * $CRONJOB")|crontab -u anno sandbox -
        fi
    fi

    # TODO Should be automated away.
    echo "PostgreSQL connection string:  " \
        "postgresql://anno sandbox:$PASSWORD@localhost/anno sandbox"
}
...
_create_virtual_machines() {
    ...
    sudo -u anno sandbox -i vmcloak init "${EGGNAME}_bird" "$WINOS" $options
    if [ "$?" -eq 0 ]; then
        sudo -u anno sandbox -i vmcloak install "${EGGNAME}_bird" $DEPENDENCIES
    fi
    ...
    for i in $(seq 1 "$VMCOUNT"); do
        ...
        if grep '"'$name'"' <(sudo -u anno sandbox -i VBoxManage list vms); then
            continue
        fi

        # As vmcloak-clone will add an entry for this node we remove it just
        # in case it did already exist.
        "/opt/anno sandbox/utils/machine.py" --delete "$name"
        ...
        sudo -u anno sandbox -i vmcloak snapshot "${EGGNAME}_bird" \
            "$name" "192.168.56.$((2+$i))"

        echo "Registering Virtual Machine $name.."
        sudo -u anno sandbox -i vmcloak register "$name" /opt/anno sandbox
    done
    ...
    if [ "$LONGTERM" -ne 0 ]; then
        sudo -u anno sandbox -i sh "$CRONJOB"
    fi
...
_setup_vms() {
    ...
    for dirname in $(cd "$source" && find * -type d); do
        sudo -u anno sandbox mkdir -p "$target/$dirname"
    done
    ...
    for filename in $(cd "$source" && find bird*.vdi */*/*.vdi */*/*.sav); do
        sudo -u anno sandbox ln -fs "$source/$filename" "$target/$filename"
    done
    ...
    for filename in $(cd "$source" && find */*.vbox); do
        sudo -u anno sandbox cp "$source/$filename" "$target/$filename"
    done
}
...
_setup_vmmount() {
    ...
    mount -o "size=$REQSIZE,uid=anno sandbox,gid=anno sandbox" -t tmpfs tmpfs "$target"

    # Copy all files from the backup to the mount.
    sudo -u anno sandbox cp -rf "$source/." "$target"
}
...
if [ "$VMCHECKUP" -eq 0 ]; then
    ...
    # VMs breaking).
    cat > /etc/default/anno sandbox-setup << EOF
...
chown anno sandbox:anno sandbox "$VMS" "$VMBACKUP" "$VMMOUNT"
