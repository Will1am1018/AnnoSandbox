...
#      without a password
#   3) Load and launch the anno sandbox guest agent (agent.py)
#
# Usage
# ---------
...
# anno sandbox agent locations
AGENT_DIR="/Users/Shared"
AGENT_URL="https://raw.githubusercontent.com/anno sandboxsandbox/anno sandbox/master/agent/agent.py"
...
echo "[INFO]: Downloading the anno sandbox guest agent"
curl -o "$AGENT_DIR"/agent.py "$AGENT_URL"
# [3.1] Install dependencies
sudo easy_install pip
(cd "$(dirname "$0")/.." && sudo -H pip install -r requirements.txt)
# [4] and run it
echo "[INFO]: Launching the anno sandbox guest agent"
