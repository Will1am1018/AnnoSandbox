...
from lib.anno sandbox.core.database import TASK_FAILED_PROCESSING, TASK_REPORTED
...
def process(target=None, copy_path=None, task=None):
    ...
    if cfg.anno sandbox.delete_bin_copy and copy_path and os.path.exists(copy_path):
        os.unlink(copy_path)
        ...
        while not maxcount or count != maxcount:
            ...
            if task.category == "file":
                ...
                copy_path = os.path.join(
                    anno sandbox_ROOT, "storage", "binaries", sample.sha256
                )
            else:
                copy_path = None
