...
from lib.anno sandbox.common.utils import store_temp_file, delete_folder
from lib.anno sandbox.core.database import Database, TASK_RUNNING, Task
from lib.anno sandbox.core.database import TASK_REPORTED, TASK_COMPLETED
from lib.anno sandbox.core.startup import drop_privileges
from lib.anno sandbox.core.rooter import rooter
...
def tasks_delete(task_id):
    ...
    if task:
        ...
        if db.delete_task(task_id):
            delete_folder(os.path.join(anno sandbox_ROOT, "storage",
                                       "analyses", "%d" % task_id))
            response["status"] = "OK"
        else:
            return json_error(500, "An error occurred while trying to "
...
def tasks_report(task_id, report_format="json"):
    ...
    if report_format.lower() in formats:
        report_path = os.path.join(anno sandbox_ROOT, "storage", "analyses",
                                   "%d" % task_id, "reports",
                                   formats[report_format.lower()])
    elif report_format.lower() in bz_formats:
        bzf = bz_formats[report_format.lower()]
        srcdir = os.path.join(anno sandbox_ROOT, "storage",
                              "analyses", "%d" % task_id)
        s = StringIO()
...
def task_screenshots(task_id=0, screenshot=None):
    folder_path = os.path.join(anno sandbox_ROOT, "storage", "analyses", str(task_id), "shots")
...
def files_get(sha256):
    file_path = os.path.join(anno sandbox_ROOT, "storage", "binaries", sha256)
...
def pcap_get(task_id):
    file_path = os.path.join(anno sandbox_ROOT, "storage", "analyses",
                             "%d" % task_id, "dump.pcap")
    if os.path.exists(file_path):
        try:
...
@app.route("/anno sandbox/status")
@app.route("/v1/anno sandbox/status")
def anno sandbox_status():
    ...
    paths = dict(
        binaries=os.path.join(anno sandbox_ROOT, "storage", "binaries"),
        analyses=os.path.join(anno sandbox_ROOT, "storage", "analyses"),
        temporary=temp_file,
    )
...
def memorydumps_list(task_id):
    folder_path = os.path.join(anno sandbox_ROOT, "storage", "analyses", str(task_id), "memory")

    if os.path.exists(folder_path):
        memory_files = []
        memory_path = os.path.join(anno sandbox_ROOT, "storage", "analyses", str(task_id), "memory")
