...
from lib.anno sandbox.core.database import Database, TASK_REPORTED, TASK_COMPLETED
from lib.anno sandbox.core.database import Task, TASK_FAILED_PROCESSING
from lib.anno sandbox.core.plugins import RunProcessing, RunSignatures, RunReporting
from lib.anno sandbox.core.startup import init_modules, drop_privileges
...
def process(target=None, copy_path=None, task=None, report=False, auto=False):
    ...
    if report:
        ...
        if auto:
            if cfg.anno sandbox.delete_original and os.path.exists(target):
                os.unlink(target)

            if cfg.anno sandbox.delete_bin_copy and copy_path and \
...
def autoprocess(parallel=1):
    maxcount = cfg.anno sandbox.max_analysis_count
    count = 0
    db = Database()
    pending_results = {}
        ...
        while True:
            ...
            for task in tasks:
                ...
                if task.category == "file":
                    ...
                    copy_path = os.path.join(anno sandbox_ROOT, "storage",
...
def main():
    ...
    log = logging.getLogger("anno sandbox.process")
