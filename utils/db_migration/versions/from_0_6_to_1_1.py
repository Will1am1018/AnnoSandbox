...
"""Database migration from anno sandbox 0.6 to anno sandbox 1.1.
...
import lib.anno sandbox.core.database as db
...
def upgrade():
    ...
    # Migrations are supported starting form anno sandbox 0.6 and anno sandbox 1.0; I need a way to figure out if from which release
    # it will start because both schema are missing alembic release versioning.
    # I check for tags table to distinguish between anno sandbox 0.6 and 1.0.
    conn = op.get_bind()

    if conn.engine.dialect.has_table(conn.engine.connect(), "machines_tags"):
        # If this table exist we are on anno sandbox 1.0 or above.
        # So skip SQL migration.
        pass
    else:
        # We are on anno sandbox < 1.0, hopefully 0.6.
...
def mongo_upgrade():
    ...
    if config.mongodb.enabled:
        ...
        database = config.mongodb.get("db", "anno sandbox")
        ...
        if "anno sandbox_schema" in db.collection_names():
            print "Mongo schema version not expected"
            sys.exit()
        else:
            db.anno sandbox_schema.save({"version": mongo_revision})
