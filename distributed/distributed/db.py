...
class Node(db.Model):
    """anno sandbox node database model."""
...
class Machine(db.Model):
    """Machine database model related to a anno sandbox node."""
    id = db.Column(db.Integer, primary_key=True)
...
class Task(db.Model, Serializer):
    ...
    # anno sandbox node and Task ID this has been submitted to.
    node_id = db.Column(db.Integer, db.ForeignKey("node.id"))
    task_id = db.Column(db.Integer)
    status = db.Column(task_status, nullable=False)

    # Timestamps for this task. When it was submitted, when it was delegated
    # to a anno sandbox node, when the analysis started, and when we retrieved
    # the report.
