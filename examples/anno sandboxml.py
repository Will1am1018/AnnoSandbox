...
try:
    ...
    print >> sys.stderr, "Some of the packages required by anno sandboxML are not \
        available."
    print >> sys.stderr, e


def init_anno sandboxml():
    """Initialise anno sandboxML analysis with default parameters."""
    cfg = Config("anno sandboxml")

    # Load reports for clustering
    loader = Loader()  # class loader
    loader.load_binaries(anno sandbox_ROOT + "/" + cfg.anno sandboxml.data_directory)
    ...
    if cfg.anno sandboxml.clustering:
        # Select features
        selected_features = []
        sf = [i.strip() for i in cfg.anno sandboxml.features.split(",")]
        if "simple" in sf:
        ...
        filters = [i.strip() for i in cfg.anno sandboxml.features_filter.split(",")]
        ...
        cl_args = \
            [i for i in cfg.anno sandboxml.clustering_parameters.split(",")]

        # Select clustering algorithm
        cls_alg = cfg.anno sandboxml.clustering_algorithm.lower()
        if cls_alg == "hdbscan":
    ...
    if cfg.anno sandboxml.abnormal_behaviour:
    ...
    if cfg.anno sandboxml.anomalies_detection:
    ...
    if cfg.anno sandboxml.compare_new_samples:
        test_location = anno sandbox_ROOT + "/" + cfg.anno sandboxml.test_directory
    ...
    if cfg.anno sandboxml.clustering and cfg.anno sandboxml.save_clustering_results:
        if cfg.anno sandboxml.clustering_results_directory:
            ml.save_clustering_results(loader, anno sandbox_ROOT + "/" + cfg.anno sandboxml. \
...
class Instance(object):
    """Machine Learning (clustering) for anno sandbox."""
    ...
    def extract_basic_features(self):
        ...
        These are extracted characteristics of the binary by anno sandbox sandbox."""
