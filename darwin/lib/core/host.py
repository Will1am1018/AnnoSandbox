...
class anno sandboxHost(object):
    """ Sending analysis results back to the anno sandbox Host.
    ...
    launch_times = {
        # Since anno sandbox host expects us to send relative times, we remember when
    ...
    def send_api(self, thing):
        """ Sends a new API notification to the anno sandbox host """
        pid = thing.pid
        api = thing.api
        ...
        if pid not in self.sockets:
            ...
            if not self.sockets[pid]:
                raise Exception("anno sandboxHost error: could not create socket.")
    ...
    def _send_new_process(self, thing):
        ...
        # Convert our unix timestamp into Windows's FILETIME because anno sandbox
