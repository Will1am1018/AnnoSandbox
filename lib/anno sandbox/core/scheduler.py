...
from lib.anno sandbox.common.config import Config, emit_options
from lib.anno sandbox.common.constants import anno sandbox_ROOT
from lib.anno sandbox.common.exceptions import anno sandboxMachineError, anno sandboxGuestError
...
from lib.anno sandbox.common.objects import File
from lib.anno sandbox.common.utils import create_folder
from lib.anno sandbox.core.database import Database, TASK_COMPLETED, TASK_REPORTED
from lib.anno sandbox.core.guest import GuestManager
from lib.anno sandbox.core.plugins import list_plugins, RunAuxiliary, RunProcessing
from lib.anno sandbox.core.plugins import RunDetection, RunSignatures, RunReporting
...
class AnalysisManager(threading.Thread):
    ...
    def init_storage(self):
        ...
        self.storage = os.path.join(anno sandbox_ROOT,
                                    "storage",
                                    "analyses",
                                    str(self.task.id))
    ...
    def store_file(self):
        ...
        sha256 = File(self.task.target).get_sha256()
        self.binary = os.path.join(anno sandbox_ROOT, "storage", "binaries", sha256)
    ...
    def build_options(self):
        ...
        options["terminate_processes"] = self.cfg.anno sandbox.terminate_processes
    ...
    def wait_finish(self):
        ...
        trying to engage with the anno sandbox Agent."""
    ...
    def launch_analysis(self):
        ...
        try:
        ...
        except anno sandboxGuestError as e:
            if not unlocked:
                machine_lock.release()
            log.error(str(e), extra={"task_id": self.task.id})
        finally:
            ...
            if self.cfg.anno sandbox.memory_dump or self.task.memory:
                try:
    ...
    def process_results(self):
        ...
        if self.task.category == "file" and self.cfg.anno sandbox.delete_original:
            if not os.path.exists(self.task.target):
        ...
        if self.task.category == "file" and self.cfg.anno sandbox.delete_bin_copy:
    ...
    def run(self):
        ...
        try:
            ...
            if self.cfg.anno sandbox.process_results:
            ...
            if hasattr(os, "symlink"):
                latest = os.path.join(anno sandbox_ROOT, "storage",
                                      "analyses", "latest")
...
class Scheduler(object):
    ...
    def initialize(self):
        ...
        machinery_name = self.cfg.anno sandbox.machinery

        max_vmstartup_count = self.cfg.anno sandbox.max_vmstartup_count
        ...
        conf = os.path.join(anno sandbox_ROOT, "conf", "%s.conf" % machinery_name)

        if not os.path.exists(conf):
            raise anno sandboxCriticalError("The configuration file for machine "
        ...
        try:
            ...
            raise anno sandboxCriticalError("Error initializing machines: %s" % e)

        # At this point all the available machines should have been identified
        # and added to the list. If none were found, anno sandbox needs to abort the
        # execution.
        if not len(machinery.machines()):
            raise anno sandboxCriticalError("No machines available.")
        else:
            log.info("Loaded %s machine/s", len(machinery.machines()))

        if len(machinery.machines()) > 1 and self.db.engine.name == "sqlite":
            log.warning("As you've configured anno sandbox to execute parallel "
                        "analyses, we recommend you to switch to a MySQL or"
                        "a PostgreSQL database as SQLite might cause some "
                        "issues.")

        if len(machinery.machines()) > 4 and self.cfg.anno sandbox.process_results:
        ...
        # anno sandbox was terminated for some reason and various forwarding rules
        ...
        for machine in machinery.machines():
            if not machine.interface:
                log.info("Unable to determine the network interface for VM "
                         "with name %s, anno sandbox will not be able to give it "
    ...
    def start(self):
        ...
        if self.maxcount is None:
            self.maxcount = self.cfg.anno sandbox.max_analysis_count
        ...
        while self.running:
            ...
            if self.cfg.anno sandbox.freespace:
                # Resolve the full base path to the analysis folder, just in
                # case somebody decides to make a symbolic link out of it.
                dir_path = os.path.join(anno sandbox_ROOT, "storage", "analyses")
                ...
                if hasattr(os, "statvfs"):
                    ...
                    if space_available < self.cfg.anno sandbox.freespace:
                        log.error("Not enough free disk space! (Only %d MB!)",
                                  space_available)
                        continue

            # Have we limited the number of concurrently executing machines?
            if self.cfg.anno sandbox.max_machines_count:
                # Are too many running?
                if len(machinery.running()) >= self.cfg.anno sandbox.max_machines_count:
                    continue
