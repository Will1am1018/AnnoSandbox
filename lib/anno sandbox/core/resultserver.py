...
from lib.anno sandbox.common.abstracts import ProtocolHandler
...
from lib.anno sandbox.common.exceptions import anno sandboxResultError
from lib.anno sandbox.common.netlog import BsonParser
from lib.anno sandbox.common.utils import create_folder, Singleton
...
class ResultServer(SocketServer.ThreadingTCPServer, object):
            ...
            try:
                ...
                if e.errno == errno.EADDRINUSE:
                    if self.cfg.resultserver.get("force_port"):
                        raise anno sandboxCriticalError(
                    ...
                    raise anno sandboxCriticalError(
                        ...
                        "usually happens when you start anno sandbox without "
                        "bringing up the virtual interface associated with "
                        "the ResultServer IP address. Please refer to "
                        "http://docs.anno sandboxsandbox.org/en/latest/faq/#troubles-problem"
    ...
    def build_storage_path(self, ip):
        ...
        return os.path.join(anno sandbox_ROOT, "storage", "analyses", str(task.id))
...
class ResultHandler(SocketServer.BaseRequestHandler):
    ...
    def negotiate_protocol(self):
        ...
        if command == "BSON":
            ...
            raise anno sandboxOperationalError(
    ...
    def handle(self):
        ...
        except anno sandboxResultError as e:
            log.warning(
                "ResultServer connection stopping because of "
                "anno sandboxResultError: %s.", e
            )
