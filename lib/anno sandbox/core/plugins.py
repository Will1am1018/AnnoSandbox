...
class RunSignatures(object):
    ...
    def __init__(self, results):
        ...
        # strip that part off.
        self.version = anno sandbox_VERSION.split("-")[0]
    ...
    def check_signature_version(self, signature):
        ...
        :return: check result.
        """
        # Check the minimum anno sandbox version for this signature, if provided.
        if signature.minimum:
            try:
                # If the running anno sandbox is older than the required minimum
                ...
                if StrictVersion(self.version) < StrictVersion(signature.minimum):
                    log.debug("You are running an older incompatible version "
                              "of anno sandbox, the signature \"%s\" requires "
                              "minimum version %s.",
                              signature.name, signature.minimum)
                    return False

                if StrictVersion("1.2") > StrictVersion(signature.minimum):
                    log.warn("anno sandbox signature style has been redesigned in "
                             "anno sandbox 1.2. This signature is not "
                             "compatible: %s.", signature.name)
                    return False

                if StrictVersion("2.0") > StrictVersion(signature.minimum):
                    log.warn("anno sandbox version 2.0 features a lot of changes that "
        ...
        # Check the maximum version of anno sandbox for this signature, if provided.
        if signature.maximum:
            try:
                # If the running anno sandbox is newer than the required maximum
                # version, skip this signature.
...
class RunReporting(object):
    ...
    def process(self, module):
        ...
        except anno sandboxReportError as e:
    ...
    def run(self):
        ...
        :raise anno sandboxReportError: if a report module fails.
        """
