...
from lib.anno sandbox.common.colors import red, green, yellow, cyan
...
from lib.anno sandbox.common.exceptions import anno sandboxStartupError, anno sandboxDatabaseError
from lib.anno sandbox.common.exceptions import anno sandboxOperationalError
from lib.anno sandbox.common.utils import create_folders
from lib.anno sandbox.core.database import Database, TASK_RUNNING
from lib.anno sandbox.core.database import TASK_FAILED_ANALYSIS, TASK_PENDING
from lib.anno sandbox.core.plugins import import_plugin, import_package, list_plugins
...
def check_python_version():
    """Checks if Python version is supported by anno sandbox.

    :raise anno sandboxStartupError: if version is not supported.
    """
    if sys.version_info[:2] != (2, 7):
        raise anno sandboxStartupError("You are running an incompatible version "
...
def check_working_directory():
    ...
    :raise anno sandboxStartupError: if directories are not properly configured.
    """
    if not os.path.exists(anno sandbox_ROOT):
        raise anno sandboxStartupError("You specified a non-existing root "
                                 "directory: {0}".format(anno sandbox_ROOT))

    cwd = os.path.join(os.getcwd(), "anno sandbox.py")
    if not os.path.exists(cwd):
        raise anno sandboxStartupError("You are not running anno sandbox from it's "
                                 "root directory")
...
def check_configs():
    ...
    :raise anno sandboxStartupError: if config files do not exist.
    """
    configs = (
        "auxiliary.conf", "avd.conf", "anno sandbox.conf", "anno sandboxml.conf",
    ...
    for config in [os.path.join(anno sandbox_ROOT, "conf", f) for f in configs]:
        if not os.path.exists(config):
            raise anno sandboxStartupError("Config file does not exist at "
...
def create_structure():
    """Creates anno sandbox directories."""
    folders = [
        "log",
        "storage",
    ...
    try:
        create_folders(root=anno sandbox_ROOT, folders=folders)
    except anno sandboxOperationalError as e:
        raise anno sandboxStartupError(e)

def check_version():
    """Checks version of anno sandbox."""
    cfg = Config()

    if not cfg.anno sandbox.version_check:
        return

    print(" Checking for updates...")

    url = "http://api.anno sandboxsandbox.org/checkversion.php"
    data = urllib.urlencode({"version": anno sandbox_VERSION})
    ...
    if anno sandbox_VERSION.endswith("-dev"):
        print(yellow(" You are running a development version! Current stable is {}.".format(
            stable_version)))
    else:
        if LooseVersion(anno sandbox_VERSION) < LooseVersion(stable_version):
            msg = "anno sandbox Sandbox version {} is available now.".format(
                stable_version)
...
def init_logging():
    ...
    fh = logging.handlers.WatchedFileHandler(os.path.join(anno sandbox_ROOT, "log", "anno sandbox.log"))
...
def init_tasks():
    ...
    for task in db.list_tasks(status=TASK_RUNNING):
        if cfg.anno sandbox.reschedule:
...
def delete_file(*rel_path):
    filepath = os.path.join(anno sandbox_ROOT, *rel_path)
    ...
    try:
        ...
        log.warning(
            ...
            "your anno sandbox setup to the latest version: %s.",
...
def init_modules(machinery=True):
    ...
    if machinery:
        import_plugin("modules.machinery." + Config().anno sandbox.machinery)
...
def init_yara():
    ...
    yara_root = os.path.join(anno sandbox_ROOT, "data", "yara")
...
def init_binaries():
    ...
    if not os.path.exists(monitor):
        raise anno sandboxStartupError(
            ...
            "anno sandbox or when you've updated your anno sandbox version by pulling "
            ...
            "over 300 anno sandbox signatures."
        )
...
def init_rooter():
    ...
    if not Config("vpn").vpn.enabled and anno sandbox.routing.route == "none":
        return
    ...
    try:
        s.connect(anno sandbox.anno sandbox.rooter)
        ...
        if e.strerror == "No such file or directory":
            raise anno sandboxStartupError(
                ...
                "and internet to none in anno sandbox.conf and enabled to no in "
                "vpn.conf)."
            )
        ...
        raise anno sandboxStartupError("Unknown rooter error: %s" % e)
...
def init_routing():
    ...
    if vpn.vpn.enabled:
        for name in vpn.vpn.vpns.split(","):
            ...
            if anno sandbox.routing.auto_rt:
    ...
    if anno sandbox.routing.route not in ("none", "internet"):
        ...
        if anno sandbox.routing.route not in vpns:
            raise anno sandboxStartupError(
    ...
    if anno sandbox.routing.internet != "none":
        if not rooter("nic_available", anno sandbox.routing.internet):
            raise anno sandboxStartupError(
                "The network interface that has been configured as dirty "
                "line is not available."
            )

        if not rooter("rt_available", anno sandbox.routing.rt_table):
            raise anno sandboxStartupError(
        ...
        # Disable & enable NAT on this network interface. Disable it just
        # in case we still had the same rule from a previous run.
        rooter("disable_nat", anno sandbox.routing.internet)
        rooter("enable_nat", anno sandbox.routing.internet)

        # Populate routing table with entries from main routing table.
        if anno sandbox.routing.auto_rt:
            rooter("flush_rttable", anno sandbox.routing.rt_table)
            rooter("init_rttable", anno sandbox.routing.rt_table,
                   anno sandbox.routing.internet)

def anno sandbox_clean():
    """Clean up anno sandbox setup.
    ...
    except anno sandboxDatabaseError as e:
    ...
    if cfg.mongodb and cfg.mongodb.enabled:
        ...
        mdb = cfg.mongodb.get("db", "anno sandbox")
        try:
    ...
    # Paths to clean.
    paths = [
        os.path.join(anno sandbox_ROOT, "db"),
        os.path.join(anno sandbox_ROOT, "log"),
        os.path.join(anno sandbox_ROOT, "storage"),
    ]
    ...
    for dirpath, dirnames, filenames in os.walk(anno sandbox_ROOT):
