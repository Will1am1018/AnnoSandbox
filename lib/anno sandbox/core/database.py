...
from lib.anno sandbox.common.config import Config, parse_options, emit_options
from lib.anno sandbox.common.constants import anno sandbox_ROOT
from lib.anno sandbox.common.exceptions import anno sandboxDatabaseError
...
from lib.anno sandbox.common.objects import File, URL, Dictionary
from lib.anno sandbox.common.utils import create_folder, Singleton, classlock, SuperLock, json_encode
...
try:
    ...
    raise anno sandboxDependencyError(
...
class Database(object):
    ...
    def __init__(self, dsn=None, schema_check=True, echo=False):
        ...
        if dsn:
            ...
            db_file = os.path.join(anno sandbox_ROOT, "db", "anno sandbox.db")
            if not os.path.exists(db_file):
                ...
                if not os.path.exists(db_dir):
                    try:
                        ...
                        raise anno sandboxDatabaseError("Unable to create database directory: {0}".format(e))
        ...
        if not hasattr(cfg, "database"):
            log.warning("It appears you don't have a valid `database` "
                        "section in conf/anno sandbox.conf, using sqlite3 instead.")

        # Create schema.
        try:
            Base.metadata.create_all(self.engine)
        except SQLAlchemyError as e:
            raise anno sandboxDatabaseError("Unable to create or connect to database: {0}".format(e))
        ...
        if not tmp_session.query(AlembicVersion).count():
            ...
            try:
                ...
                raise anno sandboxDatabaseError("Unable to set schema version: {0}".format(e))
            ...
            if last.version_num != SCHEMA_VERSION and schema_check:
                raise anno sandboxDatabaseError(
    ...
    def lock_machine(self, label=None, platform=None, tags=None):
        ...
        try:
            ...
            if not machines.count():
                raise anno sandboxOperationalError("No machines match selection criteria.")
