...
from lib.anno sandbox.common.config import Config, parse_options
from lib.anno sandbox.common.constants import anno sandbox_ROOT
from lib.anno sandbox.common.constants import anno sandbox_GUEST_PORT, anno sandbox_GUEST_INIT
from lib.anno sandbox.common.constants import anno sandbox_GUEST_COMPLETED
from lib.anno sandbox.common.constants import anno sandbox_GUEST_FAILED
from lib.anno sandbox.common.exceptions import anno sandboxGuestError
from lib.anno sandbox.common.utils import TimeoutServer
...
def analyzer_zipfile(platform, monitor):
    ...
    root = os.path.join(anno sandbox_ROOT, "analyzer", platform)
    ...
    if platform == "windows":
        dirpath = os.path.join(anno sandbox_ROOT, "data", "monitor", monitor)
...
class OldGuestManager(object):
    ...
    def wait(self, status):
        ...
        while db.guest_get_status(self.task_id) == "starting":
            ...
            if time.time() > end:
                raise anno sandboxGuestError("{0}: the guest initialization hit the "
    ...
    def upload_analyzer(self, monitor):
        ...
        try:
            ...
            raise anno sandboxGuestError("{0}: guest communication timeout: unable "
    ...
    def start_analysis(self, options, monitor):
        ...
        url = "http://{0}:{1}".format(self.ip, anno sandbox_GUEST_PORT)
        ...
        try:
            ...
            # data.
            self.wait(anno sandbox_GUEST_INIT)
            ...
            try:
                ...
                raise anno sandboxGuestError("{0}: unable to upload config to "
            ...
            if options["category"] == "file":
                try:
                    ...
                    raise anno sandboxGuestError("Unable to read {0}, error: "
                ...
                try:
                    ...
                    raise anno sandboxGuestError("{0}: unable to upload malware to "
        ...
        # exception and abort the analysis.
            ...
            raise anno sandboxGuestError("{0}: guest communication timeout, check "
    ...
    def wait_for_completion(self):
        ...
        while db.guest_get_status(self.task_id) == "running":
            ...
            if time.time() > end:
                raise anno sandboxGuestError("The analysis hit the critical timeout, terminating.")
            ...
            if status == anno sandbox_GUEST_COMPLETED:
                log.info("%s: analysis completed successfully", self.id)
                break
            elif status == anno sandbox_GUEST_FAILED:
                error = self.server.get_error()
                if not error:
                    error = "unknown error"

                raise anno sandboxGuestError("Analysis failed: {0}".format(error))
            else:
...
class GuestManager(object):
    ...
    anno sandbox Agent which features a more abstract but more feature-rich API."""

    def __init__(self, vmid, ipaddr, platform, task_id):
        self.vmid = vmid
        self.ipaddr = ipaddr
        self.port = anno sandbox_GUEST_PORT
        self.platform = platform
        self.task_id = task_id
        self.cfg = Config()
        self.timeout = None
        ...
        # We maintain the path of the anno sandbox Analyzer on the host.
    ...
    def get(self, method, *args, **kwargs):
        ...
        try:
            ...
            raise anno sandboxGuestError(
                "anno sandbox Agent failed without error status, please try "
    ...
    def start_analysis(self, options, monitor):
        ...
        if r.status_code == 501:
            # log.info("anno sandbox 2.0 features a new Agent which is more "
        ...
        if r.status_code != 200:
            log.critical(
                ...
                "anno sandbox Developers. HTTP response headers: %s",
        ...
        try:
            ...
            log.critical(
                ...
                "the anno sandbox Developers of your issue."
            )
            db.guest_set_status(self.task_id, "failed")
            return

        log.info("Guest is running anno sandbox Agent %s (id=%s, ip=%s)",
