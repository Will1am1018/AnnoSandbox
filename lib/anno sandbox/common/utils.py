...
from lib.anno sandbox.common.constants import GITHUB_URL, ISSUES_PAGE_URL
...
def create_folders(root=".", folders=[]):
    ...
    :raise anno sandboxOperationalError: if fails to create folder.
    """
...
def create_folder(root=".", folder=None):
    ...
    if folder and not os.path.isdir(folder_path):
        try:
            ...
            raise anno sandboxOperationalError("Unable to create folder: %s" %
                                         folder_path)
...
def delete_folder(folder):
    ...
    :raise anno sandboxOperationalError: if fails to delete folder.
    """
    if os.path.exists(folder):
        try:
            shutil.rmtree(folder)
        except OSError:
            raise anno sandboxOperationalError("Unable to delete folder: "
                                         "{0}".format(folder))
...
def store_temp_file(filedata, filename, path=None):
    ...
    if path:
        ...
        tmp_path = options.anno sandbox.get("tmppath", "/tmp")
        target_path = os.path.join(tmp_path, "anno sandbox-tmp")
...
def guid_name(guid):
    if not GUIDS:
        for line in open(os.path.join(anno sandbox_ROOT, "data", "guids.txt")):
            try:
...
def exception_message():
    ...
    msg = (
        "Oops! anno sandbox failed in an unhandled exception!\nSometimes bugs are "
    ...
    msg += "anno sandbox version: %s\n" % anno sandbox_VERSION
    msg += "OS version: %s\n" % os.name
    msg += "OS release: %s\n" % get_os_release()
    msg += "Python version: %s\n" % sys.version.split()[0]
    msg += "Machine arch: %s\n" % platform.machine()

    git_version = os.path.join(anno sandbox_ROOT, ".git", "refs", "heads", "master")
