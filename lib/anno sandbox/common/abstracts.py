...
# Copyright (C) 2014-2018 anno sandbox Foundation.
...
from lib.anno sandbox.common.exceptions import anno sandboxMachineError
...
from lib.anno sandbox.common.utils import create_folder
from lib.anno sandbox.core.database import Database
...
class Machinery(object):
    ...
    def pcap_path(self, task_id):
        ...
        return os.path.join(anno sandbox_ROOT, "storage", "analyses",
                            "%s" % task_id, "dump.pcap")
    ...
    def _initialize(self, module_name):
        ...
        for machine_id in mmanager_opts["machines"].strip().split(","):
            ...
            except (AttributeError, anno sandboxOperationalError) as e:
    ...
    def _initialize_check(self):
        ...
        :raise anno sandboxMachineError: if a misconfiguration or a unkown vm state is found.
        """
        try:
        ...
        for machine in self.machines():
            ...
            try:
            ...
            except anno sandboxMachineError as e:
                ...
                raise anno sandboxCriticalError(msg)

        if not self.options_globals.timeouts.vm_state:
            raise anno sandboxCriticalError("Virtual machine state change timeout "
    ...
    def shutdown(self):
        ...
        :raise anno sandboxMachineError: if unable to stop machine.
        """
    ...
    def _wait_status(self, label, state):
        ...
        :raise anno sandboxMachineError: if default waiting timeout expire.
        """
        ...
        while current not in state: # 虚拟机状态为poweroff {wrong}
            log.debug("Waiting %i anno sandboxseconds for machine %s to switch "
                      "to status %s", waitme, label, state)
            if waitme > int(self.options_globals.timeouts.vm_state):
                raise anno sandboxMachineError("Timeout hit while for machine {0} "
...
class LibVirtMachinery(Machinery):
    ...
    def __init__(self):
        if not HAVE_LIBVIRT:
            raise anno sandboxDependencyError("Unable to import libvirt")
    ...
    def _initialize_check(self):
        ...
        :raise anno sandboxMachineError: if libvirt version is not supported.
        """
        # Version checks.
        if not self._version_check():
            raise anno sandboxMachineError("Libvirt version is not supported, "
    ...
    def start(self, label, task):
        ...
        :param label: virtual machine name.
        :param task: task object.
        :raise anno sandboxMachineError: if unable to start virtual machine.
        """
        ...
        if self._status(label) != self.POWEROFF:
            ...
            raise anno sandboxMachineError(msg)
        ...
        if vm_info.snapshot and vm_info.snapshot in snapshot_list:
            ...
            try:
                ...
                raise anno sandboxMachineError("Unable to restore snapshot on "
                                         "virtual machine {0}".format(label))
            finally:
                self._disconnect(conn)
        else:
            self._disconnect(conn)
            raise anno sandboxMachineError("No snapshot found for virtual machine "
                                     "{0}".format(label))
    ...
    def stop(self, label):
        ...
        :param label: virtual machine name.
        :raise anno sandboxMachineError: if unable to stop virtual machine.
        """
        log.debug("Stopping machine %s", label)

        if self._status(label) == self.POWEROFF:
            raise anno sandboxMachineError("Trying to stop an already stopped "
        ...
        try:
            ...
            raise anno sandboxMachineError("Error stopping virtual machine "
                                     "{0}: {1}".format(label, e))
        finally:
            self._disconnect(conn)
        # Check state.
    ...
    def dump_memory(self, label, path):
        ...
        try:
            ...
            raise anno sandboxMachineError("Error dumping memory virtual machine "
                                     "{0}: {1}".format(label, e))
        finally:
            self._disconnect(conn)
    ...
    def _status(self, label):
        ...
        try:
            ...
            raise anno sandboxMachineError("Error getting status for virtual "
        ...
        if status:
            ...
            raise anno sandboxMachineError("Unable to get status for "
                                     "{0}".format(label))
    ...
    def _connect(self):
        ...
        :raise anno sandboxMachineError: when unable to connect to libvirt.
        """
        # Check if a connection string is available.
        if not self.dsn:
            raise anno sandboxMachineError("You must provide a proper "
                                     "connection string")

        try:
            return libvirt.open(self.dsn)
        except libvirt.libvirtError:
            raise anno sandboxMachineError("Cannot connect to libvirt")

    def _disconnect(self, conn):
        """
        Disconnects to libvirt subsystem.
        
        :raise anno sandboxMachineError: if cannot disconnect from libvirt.
        """
        try:
            conn.close()
        except libvirt.libvirtError:
            raise anno sandboxMachineError("Cannot disconnect from libvirt")
    ...
    def _lookup(self, label):
        ...
        :param label: virtual machine name.
        :raise anno sandboxMachineError: if virtual machine is not found.
        """
        conn = self._connect()
        try:
            vm = conn.lookupByName(label)
        except libvirt.libvirtError:
                raise anno sandboxMachineError("Cannot find machine "
                                         "{0}".format(label))
        finally:
            self._disconnect(conn)
        return vm
    ...
    def _list(self):
        ...
        :raise anno sandboxMachineError: if unable to list virtual machines.
        """
        conn = self._connect()
        try:
            names = conn.listDefinedDomains()
        except libvirt.libvirtError:
            raise anno sandboxMachineError("Cannot list domains")
        finally:
            self._disconnect(conn)
        return names
    ...
    def _get_snapshot(self, label):
        ...
        :raise anno sandboxMachineError: if cannot find current snapshot or
        ...
        try:
            ...
            raise anno sandboxMachineError("Unable to get snapshot for "
                                     "virtual machine {0}".format(label))
        finally:
            self._disconnect(conn)
...
class Instance(object):
    ...
    def load_binaries(self, data_path, first_n_byte = 2 ** 20):
        ...
        try:
            ...
            raise anno sandboxOperationalError("Error convert it into a fixed length byte sequence")
        return np.array(tmp)
...
class Signature(object):
    """Base class for anno sandbox signatures."""
    name = ""
    description = ""
    severity = 1
    order = 1
...
class Report(object):
    ...
    def set_path(self, analysis_path):
        ...
        try:
            ...
            raise anno sandboxReportError(e)
